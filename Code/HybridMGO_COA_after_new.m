function [BestF, BestX, cnvg] = HybridMGO_COA_after_new(N, MaxIter, LB, UB, dim, fobj, S_m, S_n)
    ...
    X_mgo = initialization(half_pop1, half_dim1, UB(1:half_dim1), LB(1:half_dim1));
    BestX_mgo = [];
    BestFitness_mgo = inf;

    % Initialize COA parameters (for the other half of the population and other half of the dimensions)
    X_coa = initialization(half_pop2, half_dim2, UB(half_dim1+1:end), LB(half_dim1+1:end));
    ...
    for i = 1:half_pop2
        for j = 1:half_pop1
            ...
            fitness_ff(i,j) = fobj([X_mgo(j, :), X_coa(i, :)]);
    ...
    for Iter = 1:MaxIter
        ...
        for i = 1:half_pop2
            if temp > 30
                ...
                P = 3 * rand * fitness_f(i) / fobj([zeros(1, half_dim2), Xfood]);
                if P > 2
                    Xfood = exp(-1 / P) .* Xfood;
                    for j = 1:half_dim2
        ...
        global_fitness_coa = fobj([BestX_mgo, global_position_coa]);

        for i = 1:half_pop2
            new_fitness = fobj([BestX_mgo, Xnew_coa(i, :)]);
        ...
        for i = 1:half_pop1
            ...
            Sol_Cost_mgo(i) = fobj([X_mgo(i, :), best_position_coa]);
        ...
        for i = 1:half_pop1
            ...
            [NewX_mgo, Sol_CostNew_mgo] = Boundary_Check1(NewX_mgo, fobj, LB(1:half_dim1), UB(1:half_dim1),best_position_coa);
        ...
        for i = 1:half_pop2
            fitness_f(i) = fobj([BestX_mgo, X_coa(i, :)]);
        end
        for i = 1:half_pop1
            Sol_Cost_mgo(i) = fobj([X_mgo(i, :), best_position_coa]);
        end
        
        BestX = [BestX_mgo1, best_position_coa1];
        BestF = fobj(BestX);
