function [BestF, BestX, cnvg] = HybridMGO_COA_after_dim_1to5(N, MaxIter, LB, UB, dim, fobj)
    % Split dimensions
    five_six_dim = floor(5 * dim / 6);
    ...
    lb_mgo = LB(1:five_six_dim);
    ub_mgo = UB(1:five_six_dim);
    X_mgo = initialization(N, five_six_dim, ub_mgo, lb_mgo);
    BestX_mgo = [];
    BestFitness_mgo = inf;

    % Initialize COA parameters (second half of dimensions)
    lb_coa = LB(five_six_dim+1:end);
    ub_coa = UB(five_six_dim+1:end);
    X_coa = initialization(N, dim - five_six_dim, ub_coa, lb_coa);
    cuve_f = zeros(1, MaxIter);
    global_Cov = zeros(1, MaxIter);
    BestFitness_coa = inf;
    best_position_coa = zeros(1, dim - five_six_dim);
    fitness_f = zeros(1, N);
    ...
    for i = 1:N
        ...
        Sol_Cost_mgo(i) = fobj([X_mgo(i, :), zeros(1, dim - five_six_dim)]);
        ...
        fitness_f(i) = fobj([zeros(1, five_six_dim), X_coa(i, :)]);
    ...
    for Iter = 1:MaxIter
        ...
        for i = 1:N
            ...
            cofi = Coefficient_Vector(five_six_dim, Iter, MaxIter);
            A = randn(1, five_six_dim) .* exp(2 - Iter * (2 / MaxIter));
            D = (abs(X_mgo(i, :)) + abs(BestX_mgo)) * (2 * rand - 1);
            NewX_mgo = Solution_Imp(X_mgo, BestX_mgo, lb_mgo, ub_mgo, N, cofi, M, A, D, i);
            [NewX_mgo, Sol_CostNew_mgo] = Boundary_Check(NewX_mgo, fobj, LB(1:five_six_dim), UB(1:five_six_dim));
        ...
        Xnew_coa = zeros(N, dim - five_six_dim); % Initialize new positions
        for i = 1:N
            if temp > 30
                if rand < 0.5
                    Xnew_coa(i, :) = X_coa(i, :) + C * rand(1, dim - five_six_dim) .* (xf - X_coa(i, :));
                else
                    for j = 1:dim - five_six_dim
                        z = round(rand * (N - 1)) + 1;
                        Xnew_coa(i, j) = X_coa(i, j) - X_coa(z, j) + xf(j);
                    end
                end
            else
                P = 3 * rand * fitness_f(i) / fobj([zeros(1, five_six_dim), Xfood]);
                if P > 2
                    Xfood = exp(-1 / P) .* Xfood;
                    for j = 1:dim - five_six_dim
                    ...
                    Xnew_coa(i, :) = (X_coa(i, :) - Xfood) * p_obj(temp) + p_obj(temp) .* rand(1, dim - five_six_dim) .* X_coa(i, :);
                end
            end
        end
        ...
        global_fitness_coa = fobj([zeros(1, five_six_dim), global_position_coa]);

        for i = 1:N
            new_fitness = fobj([zeros(1, five_six_dim), Xnew_coa(i, :)]);
        ...
        % Exchange solutions
        X_mgo(idx_mgo(1:num_exchange_2), 1:(five_six_dim-20)) = best_coa_solutions(:, 1:(five_six_dim-20));
        X_coa(idx_coa(1:num_exchange_1), :) = best_mgo_solutions(:, 1:(five_six_dim-20));
    end

    % Compare the best solutions from MGO and COA and display the overall best
    if BestFitness_mgo < BestFitness_coa
        BestF = BestFitness_mgo;
        BestX = [BestX_mgo, zeros(1, dim - five_six_dim)];
    else
        BestF = BestFitness_coa;
        BestX = [zeros(1, five_six_dim), best_position_coa];
    end
end
