function [BestF, BestX, cnvg] = HybridMGO_COA(N, MaxIter, LB, UB, dim, fobj)
    % Initialize MGO parameters
    lb = ones(1, dim) .* LB;
    ub = ones(1, dim) .* UB;
    X_mgo = initialization(N, dim, UB, LB);
    BestX_mgo = [];
    BestFitness_mgo = inf;

    % Initialize COA parameters
    cuve_f = zeros(1, MaxIter);
    X_coa = initialization(N, dim, UB, LB);
    ...
    for Iter = 1:MaxIter
        ...
        for i = 1:N
            ...
            [NewX_mgo, Sol_CostNew_mgo] = Boundary_Check(NewX_mgo, fobj, LB, UB);
        ...
        global_fitness_coa = fobj(global_position_coa);

        for i = 1:N
            new_fitness = fobj(Xnew_coa(i, :));
        ...
        num_exchange = ceil(0.5 * N);
        % Sort MGO population
        [~, idx_mgo] = sort(Sol_Cost_mgo, 'descend');
        best_mgo_solutions = X_mgo(idx_mgo(1:num_exchange), :);
        % Sort COA population
        [~, idx_coa] = sort(fitness_f, 'descend');
        best_coa_solutions = X_coa(idx_coa(1:num_exchange), :);
        % Exchange solutions
        X_mgo(idx_mgo(1:num_exchange), :) = best_coa_solutions;
        X_coa(idx_coa(1:num_exchange), :) = best_mgo_solutions;
...
% Define other necessary functions here (e.g., initialization, Coefficient_Vector, Solution_Imp, Boundary_Check)
